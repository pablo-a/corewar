...
/*   Updated: 2016/06/24 19:26:03 by pabril           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				sti(t_war *war, t_champ *champ)
{
	...
	reg = get_param(war, define_params_types(REG_CODE, -1, -1,
				def_opt(0, 1, 0)), ocp.first, champ);
	val1 = get_param(war, define_params_types(REG_CODE, DIR_CODE,
				IND_CODE, def_opt(1, 1, 1)), ocp.second, champ);
	val2 = get_param(war, define_params_types(REG_CODE, DIR_CODE, -1,
				def_opt(1, 1, 0)), ocp.third, champ);
